...
public class LoginEmployeeController {
  ...
  public LoginEmployeeController(AccountRepository accountRepository,UserPriorityRepository userPriorityRepository){
    this.accountRepository = accountRepository;
    this.userPriorityRepository = userPriorityRepository;
  }

  @PostMapping("/loginEmployee/{username}/{password}")
  public Boolean login(@PathVariable String username,@PathVariable String password) {
    Account account = accountRepository.findByUsername(username);
    UserPriority userPriority = userPriorityRepository.findById(2);
    if(account != null){
